...
addpath('C:\Users\bugeon\Documents\GitHub\NeuromicsCellDetection')
...
Option_detect2.Res0 = 0.01; % threshold for the cell detection
Slice2Run = 1:4;%[8:15,24:31,40:47];
% ================================================================
Channel = Channel_reg;
...
%% import cell detection from brainbow pipeline
importBrainbow

%% Prepare for correlation calculation
clear;
load run_info.mat
Pairs = {'B','blue';'R','tomato'};
Slice2Run = 1:size(slice_files,1);
% Slice2Run = [11:13,17:20];
Stack2Run = 1;

% ------------- Adjust parameters --------------
AngleRange = [-20 20 5;-10 10 21;-10 10 21];% AngleRange = [Alpha_start Alpha_end Alpha_points; Beta_start...; Gamma_...]
% AngleRange = [-5 5 11;-5 5 11;-5 5 11];% AngleRange = [Alpha_start Alpha_end Alpha_points; Beta_start...; Gamma_...]
...
Option.StepX = 1; % smaller value will give more accurate matches, but are slower
Option.StepD = 1; % smaller value will give more accurate matches, but are slower
Option.Integ =  2; % how much to integrate pixels around the plane for the stack = slice thickness
...
for j = Stack2Run % loop through stacks
    for i = Slice2Run %loop through sections
        ...
        if ~checkSameColor(this_ZStack,this_slice,Pairs)
            continue;
        end
        ...
        gg = pt_list_vol(3,:)>600 & pt_list_vol(3,:)<1300;
        pt_list_vol = pt_list_vol(:,gg);
